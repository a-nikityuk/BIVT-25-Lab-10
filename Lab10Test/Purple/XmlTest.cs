...
namespace Lab10Test.Purple
{
    ...
    public sealed class XmlTest
    {
        ...
        [TestMethod]
        public void Test_00_OOP_XmlManager()
        {
            var type = typeof(PurpleXmlFileManager<Lab9.Purple.Purple>);
        ...
        [TestMethod]
        public void Test_01_Serialize_Xml()
        {
            ISerializer<Lab9.Purple.Purple> manager = new PurpleXmlFileManager<Lab9.Purple.Purple>("test");
            var fileManager = (IFileManager)manager;

            var folder = Path.Combine(Directory.GetCurrentDirectory(), "PurpleXmlTest1");
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                Assert.IsTrue(content.Contains(_input[i]), $"Input text not found in XML for task {i}");
                Assert.IsTrue(content.Contains("Type"), $"Type field not found in XML for task {i}");
            }
        ...
        [TestMethod]
        public void Test_02_Deserialize_Xml()
        {
            ISerializer<Lab9.Purple.Purple> manager = new PurpleXmlFileManager<Lab9.Purple.Purple>("test");
            var fileManager = (IFileManager)manager;

            var folder = Path.Combine(Directory.GetCurrentDirectory(), "PurpleXmlTest2");
            Directory.CreateDirectory(folder);
            fileManager.SelectFolder(folder);
        ...
        public void Test_03_EditFile()
        {
            ...
            var folder = Path.Combine(Directory.GetCurrentDirectory(), "PurpleXmlEdit");
            Directory.CreateDirectory(folder);
